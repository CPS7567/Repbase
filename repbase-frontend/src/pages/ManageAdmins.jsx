...
import { Avatar } from "./DevDashboard";
...
// From reping.sql — gym list for the assignment dropdown
const GYM_OPTIONS = [
  { gym_id:1, gym_name:"Iron Gym"          },
  { gym_id:2, gym_name:"Pulse Fitness"     },
  { gym_id:3, gym_name:"FitZone"           },
  { gym_id:4, gym_name:"Peak Performance"  },
];
...
// ─── ADMIN FORM MODAL ─────────────────────────────────────────────────────────
function AdminModal({ admin, gyms = GYM_OPTIONS, onSave, onClose, onDelete }) {
  const isNew = !admin;
  const [form, setForm] = useState(admin ? { ...admin } : {
    name: "", email: "", phone: "", gym_id: gyms[0]?.gym_id || 1, user_id: null,
  });
...
// ─── MANAGE ADMINS PAGE ───────────────────────────────────────────────────────
// Props:
//   admins, setAdmins – state from parent
//   gyms              – gym list (to resolve gym names + populate dropdown)
//   onBack            – () => void
export default function ManageAdmins({ admins, setAdmins, gyms = GYM_OPTIONS, onBack }) {
  ...
  function handleSave(form) {
    ...
    if (editing === "new") {
      setAdmins(prev => [...prev, { ...form, admin_id: Date.now() }]);
      showToast("Admin added successfully.");
    } else {
      setAdmins(prev => prev.map(a => a.admin_id === form.admin_id ? form : a));
