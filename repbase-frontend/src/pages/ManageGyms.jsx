...
// ─── MANAGE GYMS PAGE ─────────────────────────────────────────────────────────
// Props:
//   gyms, setGyms  – state from parent
//   onBack         – () => void
export default function ManageGyms({ gyms, setGyms, onBack }) {
  ...
  function handleSave(form) {
    if (!form.gym_name.trim()) { showToast("Gym name is required."); return; }
    if (editing === "new") {
      setGyms(prev => [...prev, { ...form, gym_id: Date.now(), member_count: 0, trainer_count: 0, admin_count: 0 }]);
      showToast("Gym enrolled successfully.");
    } else {
      setGyms(prev => prev.map(g => g.gym_id === form.gym_id ? form : g));
  ...
  function handleDelete(id) {
    setGyms(prev => prev.filter(g => g.gym_id !== id));
    setEditing(null);
