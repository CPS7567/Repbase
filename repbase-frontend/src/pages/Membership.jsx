...
export default function Membership() {
  ...
  useEffect(() => {
    async function fetchMembershipData() {
      ...
      try {
        // Fetch both the user's status and the general plans catalog
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}>
          {plans.map(p => (
            <div key={p.name} style={{
              ...
              <Btn variant={p.popular ? "primary" : "ghost"} style={{ width: "100%", textAlign: "center", padding: "8px", fontSize: 12 }}>
                Select
              </Btn>
            </div>
          ))}
